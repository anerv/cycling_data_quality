...
def format_osm_style(styler):
    ...
    styler.format(formatter={' ': lambda x: f'{str(round(x))}%'}, subset=pd.IndexSlice["Largest component's share of network size",:])
    styler.set_table_styles([cell_hover, row_hover, columns_osm, caption, index_name_osm, cell_style], overwrite=False)
    styler.applymap_index(lambda v: 'color:white; font-weight:bold; background-color: purple; font-size:1em;', axis=0)
...
def format_topology_style(styler):
    ...
    styler.applymap(style_pct_value, osm_better='color:blue;',osm_worse='color:orange;',subset=topo_slice)
    styler.applymap(style_pct_value_inversed, osm_better='color:blue;',osm_worse='color:orange;',subset=topo_slice_inverse)
